          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {STATUSES.map(s => (
              <button key={s.key} onClick={() => setStatus(s.key)}
                className="badge"
                style={{ cursor:'pointer', border: status===s.key ? 'none' : '1px solid var(--paper-3)', padding:'6px 14px', fontSize:12.5, background: status===s.key ? 'var(--ink)' : 'var(--white)', color: status===s.key ? '#fff' : 'var(--ink-3)', borderRadius: 20 }}>
                {s.label}
              </button>
            ))}
          </div>
